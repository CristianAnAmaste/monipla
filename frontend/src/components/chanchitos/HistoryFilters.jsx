...
function HistoryFilters({ filters, options, catalogs, loading, onChange, onFundoChange, onCampoChange, onVariedadChange, onSubmit, onClear }) {
  ...
  return (
    <section className="rounded-xl border border-[#dbe5da] bg-white p-5 shadow-sm sm:p-6" aria-labelledby="history-filters-title">
      ...
      <form className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5" onSubmit={onSubmit} noValidate>
        ...
        <SelectField id="pageSize" label="Resultados por página" value={filters.pageSize} onChange={onChange}>
          {[10, 25, 50].map((size) => <option key={size} value={size}>{size}</option>)}
        </SelectField>
        <div className="flex items-end gap-3 sm:col-span-2 xl:col-span-5">
          <button type="button" className="rounded-lg border border-[#b8cbb8] px-4 py-2 text-sm font-semibold text-[#35563b] hover:bg-[#f2f7f0] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#39744a]" onClick={onClear}>Limpiar</button>
