class ChanchitosRepository {
  ...
  async ejecutarConsultaPdfFiltrada(filtros) {
    ...
    const result = await this.crearRequestHistorial(pool, filtros).query(`
        ...
        SELECT mb.id_catalogo_sdp, mb.fundo, mb.nombre_productor, mb.variedad, mb.cuartel, mb.codigo_trazabilidad, mb.sdp, mb.codigo_sag
        FROM dbo.MONIPLA_CATALOGO_SDP_MB mb INNER JOIN (SELECT DISTINCT id_catalogo_sdp FROM #CabecerasPdf WHERE id_catalogo_sdp IS NOT NULL) usados ON usados.id_catalogo_sdp = mb.id_catalogo_sdp;
    ...
    return { cabeceras: recordsets[0] || [], detalles: recordsets[1] || [], catalogos: recordsets[2] || [], fundos: recordsets[3] || [], campos: recordsets[4] || [], variedades: recordsets[5] || [], cuarteles: recordsets[6] || [], monitoreadores: recordsets[7] || [], estadosFenologicos: recordsets[8] || [] };
  }
